...
A GitHub Actions workflow (`.github/workflows/ci.yml`) enforces the project's quality gates on every pull request to `develop` and `master`. The pipeline runs the Python test suite, the TypeScript plugin typecheck and tests, and the OKF strict validation, using the same `uv`-based toolchain as local development. The `quality` job is a required status check for merging to protected branches.
