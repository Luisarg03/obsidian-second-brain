...
Project versioning follows strict semver, driven by conventional commits. `pyproject.toml` is the single source of truth for the version; `scripts/bump_version.py` propagates it to the plugin's `package.json` and regenerates `uv.lock`. `scripts/generate_changelog.py` produces changelog sections from conventional commits between the latest tag and HEAD. Both scripts are stdlib-only and runnable via `uv run`.
