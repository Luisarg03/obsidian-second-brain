...
The OKF (Open Knowledge Format) v0.1 knowledge bundle lives under `memory/` in the repository root and is opened as an Obsidian vault. This capability defines the directory layout, the frontmatter schema, the reserved filenames, the read-scope rule that partitions the project into bundle surface and implementation surface, and the validator script that enforces conformance.
